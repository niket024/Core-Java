...
public class WhoisAnalyzer extends JDPacketAnalyzer
{
	...
	public boolean isAnalyzable(Packet p){
            if(p instanceof TCPPacket &&
                    (((TCPPacket)p).src_port==63 || ((TCPPacket)p).dst_port==63)){
                    return true;
                }else if(p instanceof UDPPacket && (((UDPPacket)p).src_port==63 ||
			((UDPPacket)p).dst_port==63)){
                    return true;
                }else return false;
	}
